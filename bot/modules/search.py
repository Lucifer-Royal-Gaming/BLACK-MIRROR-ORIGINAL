...
def _getResult(search_results, key, message, tool):
    ...
    for index, result in enumerate(search_results, start=1):
        if tool == 'api':
            ...
            if link.startswith('magnet:'):
                msg += f"<b>🛰️Share Magnet to</b> <a href='http://t.me/share/url?url={quote(link)}'>Telegram</a><br><br>"
            else:
    ...
    path = [telegraph.create_page(
                title='Dipesh Mirror Bot Torrent Search',
                content=content
