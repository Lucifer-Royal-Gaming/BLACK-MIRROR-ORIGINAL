...
class GoogleDriveHelper:
    ...
    def deletefile(self, link: str):
        ...
        msg = ''
        try:
            res = self.service.files().delete(fileId=file_id, supportsTeamDrives=IS_TEAM_DRIVE).execute()
    ...
    def __upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded by Dipesh Mirror Bot',
            'mimeType': mime_type,
        }
    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'╭─🗂️ Fɪʟᴇɴᴀᴍᴇ ⇢ <code>{meta.get("name")}</code>\n<b>├─🕹️Size ⇢ </b>{get_readable_file_size(self.transferred_size)}'
                msg += f'\n├─⚙️ Tʏᴘᴇ ⇢ {mime_type}'
                msg += f'\n├─📚 Sᴜʙꜰᴏʟᴅᴇʀꜱ ⇢ {self.__total_folders}'
                msg += f'\n╰─📁 Fɪʟᴇꜱ ⇢ {self.__total_files}'
                buttons = ButtonMaker()
                durl = short_url(durl)
                ...
                msg += f'╭─🗂️ Fɪʟᴇɴᴀᴍᴇ ⇢ <code>{file.get("name")}</code>'
                ...
                if mime_type is None:
                    mime_type = 'File'
                msg += f'\n├─📦 Sɪᴢᴇ ⇢ {get_readable_file_size(int(meta.get("size", 0)))}'
                msg += f'\n╰─⚙️ Tʏᴘᴇ ⇢ {mime_type}'
                if INDEX_URL is not None:
    ...
    def __create_directory(self, directory_name, parent_id):
        file_metadata = {
            "name": directory_name,
            "description": "Uploaded by Dipesh Mirrors",
    ...
    def __edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            ...
            telegraph.edit_page(
                ...
                title = 'Dipesh Mirror Bot Drive Search',
                content=content
            )
        return
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            for file in response.get('files', []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    msg += f"📂 𝐅𝐢𝐥𝐞𝐧𝐚𝐦𝐞 ⇢ <code>{file.get('name')}<br>(folder)</code><br>"
                    furl = short_url(furl)
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'╭─🗂️ Fɪʟᴇɴᴀᴍᴇ ⇢ <code>{name}</code>'
