...
from bot import app, DOWNLOAD_DIR, AS_DOCUMENT, AS_DOC_USERS, AS_MEDIA_USERS, CUSTOM_FILENAME, LOGS_CHATS
        ...
        try:
            if not self.__as_doc:
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES):
                    ...
                    try:
                        for i in LOGS_CHATS:
                            app.send_photo(i, photo=self.__sent_msg.photo.file_id, caption=cap_mono)
                    except Exception as err:
                        LOGGER.error(f"Failed to forward file to log channel:\n{err}")
                else:
                    notMedia = True
